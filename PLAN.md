...
4. ✅ SSH 証明書の署名
5. ✅ HTTP サーバと構造化ログ
6. ✅ 統合テスト（実際の sshd に食わせて受理/拒否を確認）
7. ✅ GitHub Action 側のヘルパー + 本物のトークンによる self test
8. ✅ NixOS モジュール + flake（CI で `nix build` と評価を検証）

## 検査で見つかった誤り

設計時の想定が実際と違っていた箇所。同種の誤りを繰り返さないための記録。

- **`CertChecker.CheckCert` は `IsUserAuthority` を呼ばない。** 署名は検証するが
  「どの CA を信頼するか」は見ない。当初この関数で別 CA を弾けているか
  確認したつもりになっていた。認可の入口は `Authenticate` のほう。
- **x/crypto/ssh の `CertChecker` は既知でない critical option を一律拒否する。**
  `force-command` も `SupportedCriticalOptions` に宣言しないと通らない。
  OpenSSH 本体は解釈するので証明書側の問題ではないが、x/crypto/ssh で
  サーバを書く利用者には影響する。
- **OpenSSH は鍵の隣の `<key>-cert.pub` を自動で読む。** そのため
  「証明書無し」の検査が、実際には証明書付きで通っていた。
  Action 側はこの挙動を利用して `CertificateFile` の指定を不要にしている。
- **e2e が sshd の設定ミスで起動しておらず、「接続拒否」で合格していた。**
  失敗しさえすれば合格、という書き方は検査を無意味にする。失敗理由が
  認証拒否であることまで確認し、sshd が起動しない場合は skip ではなく
  fail にした。

## 未決

- 名前（`oidc-ssh-ca` は仮）
- リプレイ対策として `jti` の単回使用を強制するか
- CA 鍵の隔離（現状はプロセス内。署名をインターフェース化して別プロセスへ
  出せる形にはしてある）
- NixOS モジュールの実サーバでの運用 (CI では build と評価を検証済み)
